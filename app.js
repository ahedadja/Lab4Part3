  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $.each(data["data"]["List"], function (i, item) {
        ...
        productListAdd =
          '<div class="col-sm-6 col-md-4 col-lg-3 mt-4" id="product' +
          item["id"] +
          '">\n' +
          ...
          '                    <button class="btn btn-info float-right btn-sm" onclick="fetchOneProduct(' +
          item["id"] +
          ')">Detail</button>\n' +
          "                </div>\n" +
          '                <div class="card-footer">\n' +
          '                    <button class="btn btn-info float-right btn-sm" onclick="addToCart(' +
          item["id"] +
          ')">Add to Cart</button>\n' +
          "                </div>\n" +
          "            </div>\n" +
...
function fetchOneProduct($id) {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      product =
        '<div class="card justify-content-center pagination-centered" style="width:900px; height:auto">\n' +
        ...
        data["data"]["List"][0]["title"].substring(0, 15) +
        "..." +
        "</h3>\n" +
        '                        <div class="rating">\n' +
        '                            <button class="btn btn-info" id="comment" onclick="fetchComments(' +
        data["data"]["List"][0]["id"] +
        ')">' +
        data["data"]["List"][0]["comment_count"] +
        " comments</button>\n" +
        "                        </div>\n" +
        '                        <p class="product-description">' +
        data["data"]["List"][0]["title"] +
        "</p>\n" +
        '                        <h5 class="price">Current Price: <span>' +
        data["data"]["List"][0]["price"] +
        "</span></h5>\n" +
        '                        <h6 class="price">Screen Size: <span>' +
        data["data"]["List"][0]["screen_size"] +
        "</span></h6>\n" +
        '                        <h6 class="price">Weight: <span>' +
        data["data"]["List"][0]["item_weight"] +
        "</span></h6>\n" +
        '                        <h6 class="price">Camera: <span>' +
        data["data"]["List"][0]["camera_description"] +
        "</span></h6>\n" +
        '                        <h6 class="price">Operating System: <span>' +
        data["data"]["List"][0]["operating_system"] +
        "</span></h6>\n" +
        '                        <div class="action">\n' +
        '                            <button type="button" class="btn btn-warning" data-toggle="modal" data-target="#exampleModal" data-whatever="' +
        data["data"]["List"][0]["id"] +
    ...
    success: function (data) {
      ...
      $.each(data["data"]["List"], function (i, item) {
        commentAdd =
          ...
          item["score"] +
          '"\n' +
          '  aria-valuemin="0" aria-valuemax="5" style="width:' +
          (item["score"] / 5) * 100 +
          '%">\n' +
          "  </div>\n" +
          "</div>\n" +
